...
def employee_dashboard(request, emp_uuid):
    employee = get_object_or_404(
        Employee, emp_uuid=emp_uuid, user=request.user)
    ...
    print("\n" + "="*80)
    print(
        f"EMPLOYEE DASHBOARD DEBUG → {employee.name} ({employee.role}) | UUID: {str_uuid}")
    print("="*80 + "\n")
    ...
    if employee.role.lower() in ["teacher", "instructor", "trainer"]:
        ...
        for course in Course.objects.all():
            ...
            if isinstance(teacher_list, list) and str_uuid in teacher_list:
                ...
                print(
                    f"✓ Matched Course for Students: {course.title} → PK: {course.pk}")
        ...
        if teacher_course_pks:
            for student in Student.objects.all():
                ...
                if any(int(cid) in teacher_course_pks for cid in student_course_ids if cid):
                    ...
                    print(
                        f"STUDENTS MATCHED: {student.student_id} | {student.name} | courses: {student_course_ids}")
    ...
    for student in Student.objects.all():
        ...
        if isinstance(joint_by_value, str) and str_uuid == joint_by_value:
            ...
            print(
                f"REFERRAL MATCH (str): {student.student_id} | {student.name} | jointBy: {joint_by_value}")

        elif isinstance(joint_by_value, list) and str_uuid in joint_by_value:
            referrals.append(student)
            print(
                f"REFERRAL MATCH (list): {student.student_id} | {student.name} | jointBy: {joint_by_value}")

    # newest first sort
    referrals.sort(
        key=lambda s: s.date_of_joining or timezone.now(), reverse=True)
